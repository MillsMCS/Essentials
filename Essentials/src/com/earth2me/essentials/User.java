...
public class User extends UserData
	...
	if (getLastHealTimestamp() > 0) {
	    ...
	    if (cooldownTime.after(now) && !isAuthorized("essentials.heal.cooldown.bypass")) { throw new Exception(tl("timeBeforeHeal", DateUtil.formatDateDiff(cooldownTime.getTimeInMillis()))); }
	}
	...
	if (!permcheck || isAuthorized("essentials.eco.loan")) { return remainingBalance.compareTo(ess.getSettings().getMinMoney()) >= 0; }
	...
	if (afkEvent.isCancelled()) { return; }
	...
	if (super.isGodModeEnabled()) {
	    ...
	    if (!ess.getSettings().getNoGodWorlds()
		    .contains(this.getLocation().getWorld().getName())) { return true; }
	}
	if (isAfk()) {
