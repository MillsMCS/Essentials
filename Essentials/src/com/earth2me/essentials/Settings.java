...
public class Settings implements net.ess3.api.ISettings {
     ...
     public BigDecimal getCommandCost(String name) {
	  ...
	  if (commandCosts != null) { return EssentialsConf.toBigDecimal(commandCosts.getString(name), BigDecimal.ZERO); }
     ...
     private String _getOperatorColor() {
	  ...
	  } else if (colorName.equalsIgnoreCase("none") || colorName.isEmpty()) { return null; }
	  ...
	  if (lastResort != null) { return lastResort.toString(); }
	  return null;
     }
     ...
     public boolean isUserInSpawnOnJoinGroup(final IUser user) {
	  for (final String group : this.spawnOnJoinGroups) {
	       if (group.equals("*") || user.inGroup(group)) { return true; }
	  }
	  return false;
     }
     ...
     private Map<Pattern, Long> _getCommandCooldowns() {
	  if (!config.isConfigurationSection("command-cooldowns")) { return null; }
